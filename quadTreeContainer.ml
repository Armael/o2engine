...
let add o depth c =
  ...
  let rec constr o depth c =
	...
	(*si o est dans plusieur sous-arbre on cree un noeud avec des
	  sous arbre vide et on l'ajoute au noeud*)
	else if (depth = 0) then (vx, vy, Leaf ([o]))
	(*si o est a la profondeur max on crée un feuille et on
	  l'ajoute a la feuille*)
	...
	else c;	(*retourne le l'arbre courant si l'objet ne peut pas être ajouté*)

    | (vx, vy, Leaf (l)) ->
      if (is_in_rect vx vy o) then
	(vx, vy, Leaf (o :: l))	(*ajoute l'objet a la feuille (il est deja a la profondeur max)*)
      else
	c	(*retourne le l'arbre courant si l'objet ne peut pas être ajouté*)
...
let remove o c =
  ...
  let rec aux o c =
    ...
    | (x, y, Void) -> (x, y, Void)
    | (x, y, Leaf (l)) ->
      (*if is_in_rect x y o then (--a rajouter peut être--) *)
      let nl = (delete_list l o) in		(*retire si possible l'élément de la feuille*)
      if nl = [] then (x, y, Void) (*si il n'y a plus d'element dans la feuille retourne le vide*)
      else (x, y, Leaf (nl))
    (*else c(--a rajouter peut être--) *)
	...
	begin
	  ...
	  let newl = delete_list lo o in(*essaye de retirer l'élément sur le noeud et dans les sous arbres*)
	  if (qt4 = Void && qt3 = Void && qt2 = Void && qt1 = Void && newl = [])
	  then (x, y, Void)  
	  (*si il n'y a plus d'élément dans l'arbre retourner le vide*)
